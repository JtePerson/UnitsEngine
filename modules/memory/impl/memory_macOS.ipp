...
export namespace units {
  ...
  Memory::VirtualRegion::VirtualRegion(const uint32_t& p_size, byte_t* p_ptr) noexcept {
    ...
    const void* ptr= mmap((void*)alignDownToAllocationGranularity((size_t)p_ptr), m_size_, PROT_NONE, MAP_PRIVATE | MAP_ANON, -1, 0u);
    if (ptr != MAP_FAILED)
      m_ptr_= (byte_t*)ptr;
    else
      m_size_= 0u;
  ...
  void Memory::VirtualRegion::commit(const uint32_t& p_size, uint32_t p_offset) const noexcept {
    mprotect((void*)alignDownToPageSize((size_t)m_ptr_ + p_offset * getPageSize()), p_size * getPageSize(), PROT_READ | PROT_WRITE);
  }
} // namespace units
